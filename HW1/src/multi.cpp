...
public:
    ...
    void deposit(int src_id, int tgt_id, float amount) {
        ...
        auto& first = accountLocks[min(src_id, tgt_id)];
        auto& second = accountLocks[max(src_id, tgt_id)];
        unique_lock<mutex> lock1(first, defer_lock);
        unique_lock<mutex> lock2(second, defer_lock);
        lock(lock1, lock2);
